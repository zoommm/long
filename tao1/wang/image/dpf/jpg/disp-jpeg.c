   ...
   for (i = 0; i < new_jpeg.w/2+1;i++) 
    {
        ...
        {
            fb_pixel(fb_inf,i,j,buf32[i+j*new_jpeg.w]);
            fb_pixel(fb_inf,(new_jpeg.w-i),j,buf32[(new_jpeg.w-i)+j*new_jpeg.w]);
        }
        usleep(1000);
    }
   ...
   for (i = 0; i < new_jpeg.w/2+1;i++) 
    {
        for (j = 0; j < new_jpeg.h/2;j++) 
        {
            ...
            fb_pixel(fb_inf,(new_jpeg.w-i),j,buf32[(new_jpeg.w-i)+j*new_jpeg.w]);
            usleep(100);
            fb_pixel(fb_inf,(new_jpeg.w-i),(new_jpeg.h-j),buf32[(new_jpeg.w-i)+(new_jpeg.h-j)*new_jpeg.w]);
            fb_pixel(fb_inf,i,(new_jpeg.h-j),buf32[i+(new_jpeg.h-j)*new_jpeg.w]);
        }
        usleep(1000);
    }
*/
